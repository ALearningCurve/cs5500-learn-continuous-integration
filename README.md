...
1. What does the __runs-on__ string  
  - the `runs-on` string determines which runner github will use to run the action. In this case, we want the runner to be our self hosted runner, so we provide the value 'self-hosted'
...
3. In `main.yml`, on which branch do the jest tests run when a pull request is submitted to the deploy branch?
  - the branch that is being merged into deploy.
